...
export interface AssistantButtonProps extends React.ComponentPropsWithoutRef<'button'> {
  label?: string;
  showLabel?: boolean;
  active?: boolean;
  inverted?: boolean;
...
export const AssistantButton: React.FC<AssistantButtonProps> = (props) => {
  const { className, label, showLabel, image, active, inverted, size = 'sm', children, trailingIcon, ...rest } = props;
  ...
  return (
    <div
      className={cx(
        ...
        {
          ['w-40']: !showLabel && size === 'sm',
          ['w-52']: !showLabel && size === 'md',
          ['w-full']: showLabel,
        },
        className
      )}
    >
      <button
        ...
        type="button"
        className={cx(
          'inline-flex justify-start items-center gap-10 w-full',
          'focus-visible:ring ring-ring',
          size === 'md' ? 'h-52 p-4' : 'h-40 p-2',
          children ?
            size === 'md' ?
              'rounded-l-groups'
            : 'rounded-l-button-md'
          : size === 'md' ? 'rounded-groups'
          : 'rounded-button-md',
          inverted ?
            active ?
          ...
          : active ? 'focus-visible:bg-background-content bg-primary-surface text-light-primary font-bold'
      ...
      >
        {image ?
          <span
            className={cx(
              ...
              : active || isHover ? 'border-background-content bg-background-content'
              : 'border-transparent bg-transparent'
            )}
          >
            {image}
        ...
        {!!trailingIcon && <span className="flex justify-end items-center grow">{trailingIcon}</span>}
      </button>
      {!!children && (
        <div
          className={cx(
